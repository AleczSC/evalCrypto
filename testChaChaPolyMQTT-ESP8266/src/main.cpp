...
const char* clientName = "ESP8266testChaChaPolyMQTT_0";
...
WiFiClient ESP8266_testChaChaPoly;
PubSubClient client(ESP8266_testChaChaPoly);
...
void loop() 
{
  ...
  if(Serial.available() > 0)
  {
    ...
    client.publish(topicTagESP8266, bufferHex);
...
void callback(String topic, byte* message, unsigned int length) 
{
  ...
  if(!memcmp(topic.c_str(), topicTagESP32, strlen(topicTagESP32)))
  {
...
void reconnect()
{
  while (!client.connected()) {
    ...
    if (client.connect(clientName)) 
    {
      ...
      client.subscribe(topicTagESP32);
    }
    else {
